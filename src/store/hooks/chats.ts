...
function lastMessageText(msg, myid) {
  ...
  if (msg.message_content) {
    ...
    if (msg.message_content.startsWith("https://jitsi.sphinx.chat/"))
      return "Join Call";
    return msg.message_content;
  }
  if (msg.amount) {
    const kind =
      msg.type === constants.message_types.invoice ? "Invoice" : "Payment";
...
export function contactForConversation(
  chat: Chat,
  contacts: Contact[],
  myid: number
) {
