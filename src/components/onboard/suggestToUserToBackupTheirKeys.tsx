...
export default function SuggestToUserToBackupTheirKeys({
  onDone,
  z,
  isTheMainRender,
}) {
  ...
  const WIDTH = 150;
  const PROGRESS =
    Math.abs(timeVideoCounter - VIDEO_DURATION_SECONDS) /
          ...
          <View>
            ...
            >
              <Typography color={"white"}>
                {isVideoDone ? "Next" : timeVideoCounter}
              </Typography>
            </Button>
          </View>
        </View>
...
const useCounterToVideo: UseCounterToVideo = () => {
  const [timeVideoCounter, setTimeVideoCounter] = useState(
    VIDEO_DURATION_SECONDS
  );
