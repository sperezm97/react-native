...
export default function People(props) {
   ...
   */
  const renderContactsToShow: any = ({ item, index }) => <Contact key={index} contact={item} onPress={() => add(item.id)} selected={selected.includes(item.id)} />

  const flatListHeader: any = () => (
    <View style={styles.topBar}>
      <Text style={styles.title}>CONTACTS</Text>
      {!noInitials && (
        <TouchableOpacity style={styles.all} onPress={selectAll} accessibilityLabel='people-select-all'>
  ...
  return useObserver(() => (
    <>
      ...
      <FlatList style={styles.scroller} data={contactsToShow} renderItem={renderContactsToShow} keyExtractor={(item: any) => String(item.id)} ListHeaderComponent={flatListHeader} />
    </>
  ))
}
