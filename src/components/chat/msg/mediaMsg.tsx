...
export default function MediaMsg(props) {
  ...
  let { data, uri, loading, paidMessageText } = useCachedEncryptedFile(
    props,
    ldat,
    true
  );
  ...
  const buy = async (amount) => {
    ...
    if (!contact_id) {
      contact_id =
        chat.contact_ids && chat.contact_ids.find((cid) => cid !== props.myid);
    }
      ...
      <View collapsable={false}>
        ...
        {showPurchaseButton && (
          <>
            {purchased ? (
              <View
                style={{ ...styles.purchasedWrap, backgroundColor: theme.main }}
              >
