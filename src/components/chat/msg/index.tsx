...
const isMsgBiggerThanScreen = (msgHeight: number) =>
  msgHeight > SCREEN_HEIGHT - 300;
...
function MsgBubble(props) {
  ...
  const allowBoost =
    !isMe && !(props.message_content || "").startsWith("boost::");
  ...
  const onLongPressHandler = () => {
    if (isMsgBiggerThanScreen(msgRefHeight.current))
      setPopoverPlacement(PopoverPlacement.CENTER);
    ReactNativeHapticFeedback.trigger("impactLight", {
      enableVibrateFallback: true,
      ignoreAndroidSystemSettings: false,
    });
