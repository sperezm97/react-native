...
export default function SendPayment({ visible }) {
  ...
  const contact_id =
    chat && chat.contact_ids && chat.contact_ids.find((cid) => cid !== 1);

  const contact =
    contact_id && contacts.contacts.find((c) => c.id === contact_id);
  ...
  return useObserver(() => {
    const label =
      ui.payMode === "payment"
        ...
        : "Request Payment";
    return (
      <ModalWrap visible={visible} onClose={close}>
        ...
        <FadeView
          opacity={next === "payment" || next === "loopout" ? 1 : 0}
