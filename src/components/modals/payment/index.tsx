...
export default function SendPaymentWrap({ visible }) {
  const { ui } = useStores()
  ...
  return (
    <ModalWrap onClose={close} visible={visible}>
      {visible && <SendPayment visible={visible} close={close} />}
    </ModalWrap>
  )
}

function SendPayment({ visible, close }) {
  ...
  return useObserver(() => {
    ...
    return (
      <Portal.Host>
        <Header title={label} onClose={handleOnClose} />
