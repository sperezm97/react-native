...
export default function EditContactWrap({ visible }) {
  const { ui } = useStores()
...
function EditContact({ visible, close }) {
  ...
  return useObserver(() => (
    <Portal.Host>
      <View style={styles.header}>
        ...
        <Text style={{ ...styles.headerTitle, color: theme.title }}>{sub ? 'Recurring' : 'Edit Contact'}</Text>
        <View style={styles.subWrap}>
          {!sub && (
            <Button style={styles.subscribe} onPress={() => setSub(true)} mode='contained' dark={true}>
              <Text style={{ fontSize: 9 }}>{existingSub ? 'Subscribed' : 'Subscribe'}</Text>
            </Button>
          )}
          {sub && existingSub && existingSub.id && (
            <View style={styles.row}>
              <IconButton icon='trash-can-outline' color='grey' size={22} style={{ marginRight: 12 }} onPress={() => setShowConfirm(true)} />
              <View style={styles.row}>
