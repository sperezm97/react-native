...
export default function JoinTribeWrap({ visible }) {
  const { ui } = useStores()
  ...
  return (
    <ModalWrap onClose={close} visible={visible}>
      {visible && <JoinTribe close={close} />}
    </ModalWrap>
  )
}

function JoinTribe({ close }) {
    ...
    <Portal.Host>
      ...
      {params && (
        <View style={styles.content}>
          <Image source={hasImg ? { uri: params.img } : require('../../../android_assets/tent.png')} style={{ width: 150, height: 150, borderRadius: 75, marginTop: 15 }} resizeMode={'cover'} />
          ...
          <Button onPress={joinTribe} mode='contained' dark={true} style={{ ...styles.button, top: h - 250 }} loading={loading}>
            Join Group
          </Button>
        </View>
      )}
