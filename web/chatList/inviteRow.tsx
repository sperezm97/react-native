...
export default function InviteRow(props) {
  ...
  const isExpired = moment(invite.expires_on || new Date())
    .utc()
    .isBefore(moment())
  ...
  return (
    ...
    >
      <QrCode>
        <img
          style={{ height: 40, width: 40, backgroundColor: 'white', border: '1px solid white' }}
          src={require('../images/invite_qr.png')}
        />
      </QrCode>
      <InviteText>
        ...
        <PayMsg>
          ...
          <PaymentText>{inviteMsg(statusString, name, confirmed)}</PaymentText>
        </PayMsg>
      </InviteText>
      {dialogOpen && (
        <InviteConfirm
          ...
          loading={loading}
          price={invite.price}
          handleCloseModal={() => setDialogOpen(false)}
        />
      )}
    </Wrap>
  )
    ...
    >
      <Content bg={theme.bg}>
        ...
        <ButtonWrap>
          ...
          <Button onClick={onConfirmHandler} loading={loading} color={'secondary'} style={{ color: 'white' }}>
            Confirm
          </Button>
        </ButtonWrap>
      </Content>
