...
export default function ViewTribe() {
  ...
  async function joinTribe() {
    ...
    await chats.joinTribe({
      ...
      // owner_route_hint: tribe.owner_route_hint, // CD commented - type error
            ...
            <Dropzone multiple={false} onDrop={dropzoneUpload}>
              {({ getRootProps, getInputProps, isDragActive }) => (
                <PicDropWrap {...getRootProps()}>
                  ...
                  <Pic
                    style={{
                      backgroundImage: `url(${picsrc ? picsrc + '?thumb=true' : avatarIcon})`,
                    }}
                  />
                </PicDropWrap>
              )}
            </Dropzone>
            <Text style={{ color: theme.greyText }}>My Picture in this Tribe</Text>
          </PicWrap>
        )}
        {alreadyJoined ? (
          <Button
