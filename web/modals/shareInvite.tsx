...
export default function ShareInvite() {
  ...
  return (
    ...
    >
      <Content bg={theme.bg}>
        <Title>SHARE INVITATION CODE</Title>
        <CopyWrap
          onClick={() => {
            navigator.clipboard.writeText(ui.shareInviteString), onCopy('Code')
          }}
        >
          <CopyClick>CLICK TO COPY</CopyClick>
          <Qr>
            <QRCode value={ui.shareInviteString} />
          </Qr>
          <Text>{ui.shareInviteString}</Text>
        </CopyWrap>
        {alert && (
          <Alert
            style={{
