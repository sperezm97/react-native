...
export default function Head({ height, appMode, appURL, setAppMode, messagePrice, status, tribeParams }) {
  ...
  return useObserver(() => {
    ...
    function openTribeInfo() {
      ...
      console.log('setTribeInfo placeholder')
      // ui.setTribeInfo(chat, tribeParams)
    }

    return (
      <Wrap style={{ background: theme.bg, height }}>
        {!chat && !showURL && <Placeholder>Open a conversation to start using Sphinx</Placeholder>}
        {chat && (
          <Inner>
            <Left>
              <AvatarWrap>
              ...
              <ChatInfo>
                <NameWrap>
                  <Name
                    onClick={chat && chat.type === constants.chat_types.conversation ? viewContact : openTribeInfo}
                    style={{ cursor: 'pointer' }}
                  >
                    {chat.name}
                  </Name>
                  {status && chat.type !== constants.chat_types.group && (
                    <Tooltip title={status === 'active' ? 'Route Confirmed' : 'Cant Find Route'} placement='right'>
                      <LockIcon
                        style={{
                ...
                <SatWrap>
                  ...
                  {(tribeParams && tribeParams.escrow_amount ? true : false) && (
                    <Price>&nbsp;{` - Amount to Stake: ${tribeParams.escrow_amount} sat`}</Price>
                  )}
                </SatWrap>
              </ChatInfo>
            </Left>
        ...
        <Right>
          ...
          {appURL && (
            <>
              ...
              {appMode ? (
                <ChatIcon
                  style={{ color: 'white', fontSize: 27, marginRight: 15, cursor: 'pointer' }}
            ...
            <>
              ...
              {showURL ? (
                ...
                <IconzWrap>
                  ...
                  {!chat && (
                    <Btn onClick={() => ui.toggleBots(ui.showBots ? false : true)}>
                      <BotIcon />
                    </Btn>
                  )}
                </IconzWrap>
