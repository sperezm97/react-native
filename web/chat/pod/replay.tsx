...
export default function Replay(props) {
  ...
  return (
    <ReplayWrap
      style={{ background: position > 0 && msgsToShow && msgsToShow[0] ? 'rgba(0, 0, 0, 0.7)' : 'transparent' }}
    >
      {position > 0 &&
        msgsToShow &&
        msgsToShow.map((m, i) => {
          return (
            <div key={i} style={{ alignSelf: m.isMe ? 'flex-end' : 'start' }}>
              {!m.isMe && <span style={{ color: getColor(m.alias), marginLeft: 8 }}>{m.alias}</span>}
              {m.amount && (
                <BoostWrap bg={m.isMe ? theme.highlight : theme.notMeMessage}>
                  {m.type === 'boost' && (
              ...
              {m.text && <ClipWrap bg={theme.highlight}>{m.type === 'clip' && <span>{m.text}</span>}</ClipWrap>}
            </div>
          )
        })}
    </ReplayWrap>
