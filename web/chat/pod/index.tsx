  ...
  async function loadPod() {
    ...
    if (thepod) {
      ...
      if (!isPlaying && chat.meta && chat.meta.itemID) {
        ...
        const episode =
          thepod.episodes &&
          thepod.episodes.length &&
          thepod.episodes.find((e) => e.id === chat.meta.itemID);
        if (episode) {
      ...
      if (!isSet) {
        ...
        if (isPlaying && eid) {
          episode =
            thepod.episodes &&
            thepod.episodes.length &&
            thepod.episodes.find((e) => e.id === eid);
        }
        if (!episode) {
          episode =
            thepod.episodes && thepod.episodes.length && thepod.episodes[0];
        }
    ...
    <PodWrap bg={theme.bg} ref={scrollRef} hide={!chatID || !url}>
      ...
      {pod && pod.episodes && (
        <PodEpisodes>
          ...
          >{`Episodes: ${pod.episodes.length}`}</div>
          <EpisodeList>
