...
export default function Pod({ url, chat, onBoost }) {
  ...
  async function loadPod() {
    ...
    if (thepod) {
      ...
      if (!isPlaying && chat.meta && chat.meta.itemID) {
        ...
        const episode =
          thepod.episodes && thepod.episodes.length && thepod.episodes.find((e) => e.id === chat.meta.itemID)
        if (episode) {
  ...
  async function sendPayments(ts: number) {
    ...
    const memo = JSON.stringify(sp)
    feed.sendPayments(dests, memo, item.price || pricePerMinute, item.chatID, true)
  }
  ...
  function onClipPayment(d) {
    if (d.pubkey && d.ts) {
      ...
      const memo = JSON.stringify(sp)
      const finalDests: Destination[] = dests.concat(extraDest) // TODO: Why unused?
      feed.sendPayments(dests, memo, pricePerMinute, chatID, false)
    }
  }
  ...
  if (pod && showStats) {
    return (
      <PodWrap bg={theme.bg} ref={scrollRef} style={{ padding: 16 }}>
        <Stats pod={pod} onClose={() => setShowStats(false)} incomingPayments={incomingPayments} earned={earned} />
      </PodWrap>
    )
  }
  ...
  return (
    <PodWrap bg={theme.bg} ref={scrollRef} hide={!chatID || !url}>
      ...
      {pod ? (
        <PodInfo>
          <PodText>
            {episode && <PodEpisode>{episode.title}</PodEpisode>}
      ...
      {insfBalance && (
        <Alert style={{ position: 'absolute', bottom: '43%', left: 'calc(50% - 80px)', opacity: 0.9 }} icon={false}>
          Insufficient Balance
        </Alert>
      )}

      <Player pod={pod} episode={episode} sendPayments={sendPayments} boost={boost} chat={chat} ppm={ppm} />
        ...
        <PodEpisodes>
          ...
          <EpisodeList>
            {pod.episodes.map((e, i) => {
              ...
              return (
                <ListedEpisode onClick={() => selectEpisode(e)} key={i}>
