...
export default function msgMenu({
  anchorEl,
  menuMessage,
  isMe,
  handleMenuClose,
  ...
  return (
    ...
    >
      <MenuItem
        onClick={() => {
          navigator.clipboard.writeText(menuMessage.message_content), handleMenuClose(), onCopy('Text')
        }}
      ...
      {link && (
        <MenuItem
          onClick={() => {
            navigator.clipboard.writeText(link), handleMenuClose(), onCopy('Link')
          }}
          style={{ fontSize: 14, color: 'white', backgroundColor: isMe ? theme.highlight : theme.extraDeep }}
        >
          <LinkIcon style={{ fontSize: 'medium', marginRight: 8 }} />
          Copy Link
        </MenuItem>
      )}
      ...
      <MenuItem
        onClick={() => {
          ui.setReplyUUID(menuMessage.uuid), handleMenuClose()
        }}
        style={{ fontSize: 14, color: 'white', backgroundColor: isMe ? theme.highlight : theme.extraDeep }}
      >
        <ReplyIcon style={{ fontSize: 'medium', marginRight: 8 }} />
        Reply
      </MenuItem>

      {!isMe && (
        <MenuItem style={{ fontSize: 14, color: 'white', backgroundColor: isMe ? theme.highlight : theme.extraDeep }}>
          <div
            onClick={() => {
              onBoost(menuMessage.uuid), handleMenuClose()
            }}
