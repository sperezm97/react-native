  ...
  if (hasLink) {
    return (
      <Wrap {...props}>
        ...
        <ReactTinyLink cardSize='small' showGraphic={true} maxLine={2} minLine={1} url={link} />
      </Wrap>
    )
  }

  const isTribe = message_content && message_content.startsWith('sphinx.chat://?action=tribe')
  if (isTribe) {
...
function Wrap(props: {
  ...
  return (
    <WrapDiv style={style}>
      {children}
      {(boosts_total_sats ? true : false) && <BoostRow {...props} isMe={isMe} />}
    </WrapDiv>
  )
}
