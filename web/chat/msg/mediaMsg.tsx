...
export default function MediaMsg(props) {
  ...
  return (
    <Wrap>
      ...
      {amt && showStats && (
        <PriceWrap>
          <PriceIcon theme={theme}>{amt} SAT</PriceIcon>
          <PriceIcon theme={theme}>{sold ? 'Purchase Succeeded' : 'Pending'}</PriceIcon>
        </PriceWrap>
      )}
      ...
      {needsPurchase && !isPaidMsg ? (
        ...
        <Media
          ...
          data={data}
          filename={filename}
          onClick={() => ui.setImgViewerParams({ data, type: media_type })}
        />
      )}
      ...
      {isPaidMsg && (
        <>
          {showPayToUnlockMessage && <Content>Pay to unlock message</Content>}
          {loading && (
            <LoadingWrap>
              <CircularProgress size={17} style={{ color: 'white' }} />
            </LoadingWrap>
          )}
          {paidMessageText && <Content style={{ paddingTop: 40 }}>{paidMessageText}</Content>}
        </>
      )}

      {showPurchaseButton && (
        <BuyButton style={{ background: theme.primary }} onClick={() => setConfirm(true)} purchased={purchased}>
      ...
      <Dialog onClose={() => setConfirm(false)} open={confirm}>
        <DialogContent>
          ...
          <Button color={'secondary'} onClick={() => buy(amt)}>
            Yes
          </Button>
        </DialogContent>
      </Dialog>
...
function Media({ type, data, onClick, filename }) {
  ...
  if (typ.startsWith('video')) {
    return (
      <VidWrap>
        <Player>
          <source src={data} type={type === 'video/mov' || type === 'video/quicktime' ? 'video/mp4' : type} />
        </Player>
