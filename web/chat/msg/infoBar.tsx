...
export default function InfoBar(props) {
  ...
  return (
    <Wrap style={{ flexDirection: isMe ? 'row-reverse' : 'row' }}>
      {senderAlias && !isMe && <Alias style={{ color: getColor(senderAlias) }}>{senderAlias}</Alias>}
      <Time>{moment(props.date).format(timeFormat)}</Time>
      {showLock && <LockIcon style={{ color: 'grey', fontSize: 11, marginLeft: 5, marginRight: 5 }} />}
      {isMe && isReceived && <FlashIcon style={{ color: '#48c998', fontSize: 12 }} />}
    </Wrap>
  )
}
